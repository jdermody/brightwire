...
namespace BrightData.Buffers2
{
    public abstract class HybridBufferBase<T> : IHybridBuffer<T>
    {
        ...
        private readonly uint _maxDistinct = 0;
        private int _index = 0;

        protected HybridBufferBase(IProvideTempStreams tempStream, uint maxCount, uint? maxDistinct)
        {
        ...
        protected Span<T> _GetTempBuffer() => ((Span<T>) _tempBuffer).Slice(_index);
        ...
        {
            var tempStream = _tempStream.Get(_id);
            var ptr = _GetTempBuffer();
            if (ptr.Length > 0)
                _WriteTo(ptr, tempStream);
