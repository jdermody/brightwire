...
namespace BrightData.Cuda.CudaToolkit
{
    ...
    public class CudaDeviceVariable<T> : IDisposable where T : struct
    {
        ...
        public CudaDeviceVariable(CUlibrary library, string name)
        {
            _devPtr = new CUdeviceptr();
            SizeT _sizeInBytes = new SizeT();
            res = DriverAPINativeMethods.LibraryManagement.cuLibraryGetGlobal(ref _devPtr, ref _sizeInBytes, library, name);
            Debug.WriteLine(String.Format("{0:G}, {1}: {2}. Name: {3}, Size (in bytes): {4}", DateTime.Now, "cuLibraryGetGlobal", res, name, _sizeInBytes.ToString()));
            if (res != CUResult.Success) throw new CudaException(res);
        ...
        SizeT MinSizeWithType(SizeT value)
        {
