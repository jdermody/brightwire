...
namespace BrightWire.Linear
{
    ...
    /// </summary>
    internal class MultinomialLogisticRegressionClassifier : ITableClassifier
    {
        ...
        public IEnumerable<(uint RowIndex, (string Classification, float Weight)[] Predictions)> Classify(IDataTable table)
        {
            ...
            for (uint i = 0; i < matrix.RowCount; i++) {
                using var rowOutput = table.Context.CreateVector(output.Select(v => v[i]).ToArray());
                using var softmax = rowOutput.Softmax();
                var classification = _model.Classification.Zip(softmax.Values).ToArray();
