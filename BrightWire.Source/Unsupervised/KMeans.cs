...
namespace BrightWire.Unsupervised
{
	...
	class KMeans : IDisposable
	{
		...
		//			.Select(c => (c, _distance.GetAverageFromData(c)))
		//			.ToList()
		//		;

		//	// check if the clusters have converged
		//	var differenceCount = _clusters.Zip(clusters, (c1, c2) => c1.Cluster.EuclideanDistance(c2.Item2) < BoundMath.ZERO_LIKE ? 0 : 1).Sum();
		//	if (differenceCount == 0)
		//		return false;

		//	_clusters = clusters;
		//	_distance.UpdateComparisonVectors(clusters.Select(d => d.Item2).ToList());
		//	return true;
		//}
