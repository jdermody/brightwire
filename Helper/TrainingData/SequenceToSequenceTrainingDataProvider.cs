...
namespace BrightWire.Helper.TrainingData
{
    public class SequenceToSequenceTrainingDataProvider : ISequentialTrainingDataProvider
    {
        ...
        class MiniBatch : ISequentialMiniBatch
        {
            ...
            public MiniBatch(IMatrix[] input, IMatrix[] output, int[] rows, Func<IReadOnlyList<IMatrix>, IMatrix, IMatrix> errorSignal)
            {
                Input = input;
            ...
            {
                return _errorSignal(output, _expectedOutput[k]);
                //return _expectedOutput[k];
            }
        }

        public SequenceToSequenceTrainingDataProvider(ILinearAlgebraProvider lap, IReadOnlyList<Tuple<float[], float[]>[]> data, Func<IReadOnlyList<IMatrix>, IMatrix, IMatrix> errorSignal)
        {
            _lap = lap;
        ...
        public ISequentialMiniBatch GetTrainingData(int sequenceLength, IReadOnlyList<int> rows)
        {
            ...
            return new MiniBatch(input, output, rows.Select(r => dataGroup[r].Item2).ToArray(), _errorSignal);
        }
    }
}
